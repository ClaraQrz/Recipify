...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _trocarFoto() async {
    ...
    final usuarioId = AuthService.instance.usuarioLogado?['id']?.toString() ?? '';
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('foto_perfil_$usuarioId', picked.path);
  ...
  Future<void> _carregarContagens() async {
    try {
      ...
      final totalItens = await EstoqueRepository.instance.contarItens(usuarioId);
      setState(() {
                  ...
                  children: [
                    ...
                    _carregando
                        ...
                        : Row(
                            ...
                            children: [
                              _ContadorItem(valor: _totalReceitas, label: 'Receitas'),
                              _Divisor(),
                              _ContadorItem(valor: _totalListas, label: 'Listas'),
                              _Divisor(),
                              _ContadorItem(valor: _totalItens, label: 'Itens'),
                            ],
                          ),
                  ],
                ),
                    ...
                    children: [
                      ...
                      ValueListenableBuilder<ThemeMode>(
                        ...
                        builder: (_, mode, __) => ListTile(
                          ...
                          title: Text(
                            'Tema',
                            style: TextStyle(color: AppTheme.textDarkOf(context), fontSize: 15),
                          ),
                          trailing: Switch(
                      ...
                      _MenuItem(
                        ...
                        onTap: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Compartilhar perfil — em breve!')),
                          );
                        },
                      ),
                      const Divider(height: 1, indent: 56),
                      _MenuItem(
                        ...
                        onTap: () {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Avaliar app — em breve!')),
                          );
                        },
                      ),
                      const Divider(height: 1, indent: 56),
...
class _ContadorItem extends StatelessWidget {
  ...
  const _ContadorItem({required this.valor, required this.label});
...
class _MenuItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      leading: Icon(icon, color: AppTheme.textDarkOf(context), size: 22),
      title: Text(
        label,
        style: TextStyle(color: AppTheme.textDarkOf(context), fontSize: 15),
      ),
